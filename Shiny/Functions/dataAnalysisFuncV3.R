...
#-------------------------------------------------------------------------------
gePlayersFich <- function(season, seasonType, path, DT = dicoPlayerMinute) {
  #-------------------------------------------------------------------------------
  # @ variables :
...
#-------------------------------------------------------------------------------
getClassicPlayerStats <- function(selectedTeam, DTgamesPlayed, DT) {
  #-------------------------------------------------------------------------------
  # @ variables :
  ...
  statsDT <- merge(timeDT, totPointsDT, by = c("player"), all.x = TRUE, all.y = FALSE) %>%
    merge(., totAssistsDT, by = c("player"), all.x = TRUE, all.y = FALSE) %>%
    merge(., totReboundsDT, by = c("player"), all.x = TRUE, all.y = FALSE) %>%
    merge(., totTODT, by = c("player"), all.x = TRUE, all.y = FALSE) %>%
    merge(., totStealsDT, by = c("player"), all.x = TRUE, all.y = FALSE) %>%
    merge(., totBlocksDT, by = c("player"), all.x = TRUE, all.y = FALSE)
...
#-------------------------------------------------------------------------------
getPlayerClassicStatsChart <- function(selectedTeam, selectedPlayer, DTgamesPlayed, DT) {
  #-------------------------------------------------------------------------------
  if (selectedTeam != "Team" & selectedPlayer != "Player") {
    
    # On vérifie que le nombre de match min est respecté
    teamStatsDT <- getClassicPlayerStats(selectedTeam, DTgamesPlayed, DT)
    
    # We download team players stats
    nbTeamPlayers <- nrow(teamStatsDT)
    
    # We filter with selected team and selected player
    playerStatsDT <- teamStatsDT[player == selectedPlayer]
...
getHistPlayerStats <- function(selectedTeam, selectedPlayer, startDate = NULL, endDate = NULL, DTcalendar, DT) {
  ...
  colnames(histStatsDT) <- c("Date", "Place", "Rslt", "Vs", "Pts", "Asts", "Rebs", "Stls", "Blks", "TOs")
  ...
  # We calculate anothe DT with mean in a single row
  histMeanDT <- data.table("Wins" = nrow(histStatsDT[Rslt == "W"]), "Losses" = nrow(histStatsDT[Rslt == "L"]), Pts = round(mean(histStatsDT[, Pts]), 2), 
                           Asts = round(mean(histStatsDT[, Asts]), 2), Rebs = round(mean(histStatsDT[, Rebs]), 2), Stls = round(mean(histStatsDT[, Stls]), 2), 
                           Blks = round(mean(histStatsDT[, Blks]), 2), TOs = round(mean(histStatsDT[, TOs]), 2))
  
  return(list(histStatsDT[order(-Date)], histMeanDT))
...
getPlayerPeriodStatsChart <- function(selectedTeam, selectedPlayer, DTplayer) {
  ...
  fig <- plotly::add_markers(p = fig, data = playerPtsMinDT, type = "scatter",
                          ...
                          #texttemplate = ~paste("<b>", TeamPointsPerPeriod, "</b><br>ppq"),
