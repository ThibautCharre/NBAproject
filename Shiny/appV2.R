...
server <- function(input, output, session) {
  ...
  output$test <- renderText({
    paste(shortNameSelected3(), playerSelected3())
  })
  ...
  # - PLAYER STATS DT
  playerStatsDT3 <- reactive({
    getLeaguePlayerStats(shortNameSelected3(), playerSelected3(), DTgamesPlayed = dicoPlayerMinute(), DT = nbaDatasDTmerged())
  })
